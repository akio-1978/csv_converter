...
|-p|--parameters|KEY=VALUE形式の文字列 複数指定可||テンプレートに直接引数を与えます<br>この値はテンプレート中で`params.KEY`として参照できます|
|-h|--help|||ヘルプを表示します|
...
``` python
{
    ...
    'rows' : [list:str], # csvの各行をdictで格納したlistです
                         # ヘッダを読み込んでいれば rows[n].ヘッダ項目名 で格納されています
                         # ヘッダを読み込んでいない場合、rows[n].col00から順に番号が振られています
    'params' : [dict] # 起動時にオプション--parametersで指定された変数が格納されます
}
```
...
#### 基本構文
``` sh
j2shrine excel [テンプレート] [excelファイル] [読込シート] [読込箇所] options...
```
##### 位置引数
...
``` python
{
    'sheets' : [ # 読み込んだシートを表します
        'name' : [str] # シートの名前です
        'rows' : [list:dict], # シート中の各行をdictで格納したlistです Ex rows[n].A
